...
{//systick handler
    connect(&timer_, &QTimer::timeout, this, &SimulationController::tick);//信号槽
}
...
void SimulationController::setup(HeatMapWidget* view)
{
    ...
    solver_.initialize(smoke_.nx(), smoke_.ny());
...
void SimulationController::start()
{
    if (running_) return;
    running_ = true;
    timer_.start(20);//20 ms 是 界面刷新周期
}
